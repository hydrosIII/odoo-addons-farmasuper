...
class TrialBalanceReport(models.TransientModel):
    ...
    **** TrialBalanceReportPartner
    ...
    _name = 'report_trial_balance_contabilidad_cfdi'
    _inherit = 'account_financial_report_contabilidad_cfdi_abstract'
    ...
    general_ledger_id = fields.Many2one(
        comodel_name='report_general_ledger_contabilidad_cfdi'
    )

    # Data fields, used to browse report data
    account_ids = fields.One2many(
        comodel_name='report_trial_balance_account_contabilidad_cfdi',
        inverse_name='report_id'
    )


class TrialBalanceReportAccount(models.TransientModel):
    _name = 'report_trial_balance_account_contabilidad_cfdi'
    _inherit = 'account_financial_report_contabilidad_cfdi_abstract'
    _order = 'sequence, code ASC, name'

    report_id = fields.Many2one(
        comodel_name='report_trial_balance_contabilidad_cfdi',
        ondelete='cascade',
        index=True
    )
    ...
    compute_account_ids = fields.Many2many(
        'account.account',relation='account_report_trial_balance_account_contabilidad_cfdi_rel',
    ...
    partner_ids = fields.One2many(
        comodel_name='report_trial_balance_partner_contabilidad_cfdi',
...
class TrialBalanceReportPartner(models.TransientModel):
    _name = 'report_trial_balance_partner_contabilidad_cfdi'
    _inherit = 'account_financial_report_contabilidad_cfdi_abstract'
...
ORDER BY
    CASE
        WHEN "report_trial_balance_partner_contabilidad_cfdi"."partner_id" IS NOT NULL
        THEN 0
        ELSE 1
    END,
    "report_trial_balance_partner_contabilidad_cfdi"."name"
        """
...
class TrialBalanceReportCompute(models.TransientModel):
    ...
    _inherit = 'report_trial_balance_contabilidad_cfdi'
    ...
    def print_report(self, report_type):
        ...
        if report_type == 'xlsx':
            ...
            report_name = 'contabilidad_cfdi.' \
                          'report_trial_balance_qweb_contabilidad_cfdi'
    ...
    def _get_html(self):
        ...
        if report:
            ...
            if context.get('is_account_hirarchy_report'):
                result['html'] = self.env.ref(
                'contabilidad_cfdi.report_account_account_hirarchy_contabilidad_cfdi').render(rcontext)
            else:
                result['html'] = self.env.ref(
                'contabilidad_cfdi.report_trial_balance_contabilidad_cfdi').render(rcontext)
        return result
    ...
    def compute_data_for_report(self):
        ...
        model = self.env['report_general_ledger_contabilidad_cfdi']
        if self.filter_account_ids:
            account_ids = self.filter_account_ids
        else:
            account_ids = self.env['account.account'].search(
                [('company_id', '=', self.company_id.id)])
        if self._context.get('is_cuentas_de_orden'):
            account_ids = account_ids.filtered(lambda x: not x.group_id or not x.group_id.code_prefix.startswith('8'))
...
INSERT INTO
    report_trial_balance_account_contabilidad_cfdi
    (
    report_id,
    create_uid,
    create_date,
...
FROM
    account_account acc
    LEFT OUTER JOIN report_general_ledger_account_contabilidad_cfdi AS rag
...
INSERT INTO
    report_trial_balance_partner_contabilidad_cfdi
    (
    report_account_id,
    create_uid,
    create_date,
...
FROM
    report_general_ledger_partner_contabilidad_cfdi rpg
INNER JOIN
    report_general_ledger_account_contabilidad_cfdi rag ON rpg.report_account_id = rag.id
INNER JOIN
    report_trial_balance_account_contabilidad_cfdi ra ON rag.code = ra.code
WHERE
    rag.report_id = %s
AND ra.report_id = %s
        """
...
WITH computed AS (WITH RECURSIVE cte AS (
   ...
   FROM   report_trial_balance_account_contabilidad_cfdi
   WHERE report_id = %s
   GROUP BY report_trial_balance_account_contabilidad_cfdi.id
   ...
   FROM   cte c
   JOIN   report_trial_balance_account_contabilidad_cfdi p USING (parent_id)
    WHERE p.report_id = %s
)
...
UPDATE report_trial_balance_account_contabilidad_cfdi
...
FROM computed
WHERE report_trial_balance_account_contabilidad_cfdi.account_group_id = computed.account_group_id
    AND report_trial_balance_account_contabilidad_cfdi.report_id = %s
"""
    ...
    def _add_account_group_account_values(self):
        """Compute values for report_trial_balance_account group in child."""
        query_update_account_group = """
DROP AGGREGATE IF EXISTS array_concat_agg_contabilidad_cfdi(anyarray);
CREATE AGGREGATE array_concat_agg_contabilidad_cfdi(anyarray) (
  SFUNC = array_cat,
  STYPE = anyarray
);
...
SELECT account_group_id,
    array_concat_agg_contabilidad_cfdi(DISTINCT child_account_ids)::int[] as child_account_ids
FROM   cte
GROUP BY cte.account_group_id, cte.child_account_ids
ORDER BY account_group_id
)
SELECT account_group_id,
    array_concat_agg_contabilidad_cfdi(DISTINCT child_account_ids)::int[]
        AS child_account_ids from computed
GROUP BY account_group_id)
UPDATE report_trial_balance_account_contabilidad_cfdi
SET child_account_ids = aggr.child_account_ids
FROM aggr
WHERE report_trial_balance_account_contabilidad_cfdi.account_group_id = aggr.account_group_id
    AND report_trial_balance_account_contabilidad_cfdi.report_id = %s
"""
        query_update_account_params = (self.id, self.id, self.id,)
        self.env.cr.execute(query_update_account_group,
    ...
    def _update_account_group_computed_values(self):
        """Compute values for report_trial_balance_account_contabilidad_cfdi group in compute."""
        query_update_account_group = """
...
(SELECT
 ...
 FROM
    ...
    LEFT OUTER JOIN report_trial_balance_account_contabilidad_cfdi AS ra
...
FROM accgroup
WHERE report_trial_balance_account_contabilidad_cfdi.account_group_id = accgroup.id
"""
...
FROM report_trial_balance_account_contabilidad_cfdi as newline
WHERE newline.account_group_id = report_trial_balance_account_contabilidad_cfdi.parent_id
    AND report_trial_balance_account_contabilidad_cfdi.report_id = newline.report_id
    AND report_trial_balance_account_contabilidad_cfdi.account_id is not null
    AND report_trial_balance_account_contabilidad_cfdi.report_id = %s"""
        query_update_account_params = (self.id,)
        self.env.cr.execute(query_update_account_group,
                            query_update_account_params)
