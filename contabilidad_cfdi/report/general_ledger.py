...
class GeneralLedgerReport(models.TransientModel):
    ...
    **** GeneralLedgerReportMoveLine
    ...
    _name = 'report_general_ledger_contabilidad_cfdi'
    ...
    filter_cost_center_ids = fields.Many2many(
        comodel_name='account.analytic.account',relation='analytic_account_report_general_ledger_contabilidad_cfdi'
    )
    filter_analytic_tag_ids = fields.Many2many(
        comodel_name='account.analytic.tag',relation='ana_account_tag_report_general_ledger_contabilidad_cfdi'
    )
    ...
    account_ids = fields.One2many(
        comodel_name='report_general_ledger_account_contabilidad_cfdi',
...
class GeneralLedgerReportAccount(models.TransientModel):
    ...
    _name = 'report_general_ledger_account_contabilidad_cfdi'
    _inherit = 'account_financial_report_contabilidad_cfdi_abstract'
    _order = 'code ASC'
    ...
    move_line_ids = fields.One2many(
        comodel_name='report_general_ledger_move_line_contabilidad_cfdi',
        inverse_name='report_account_id'
    )
    partner_ids = fields.One2many(
        comodel_name='report_general_ledger_partner_contabilidad_cfdi',
        inverse_name='report_account_id'
    )
...
class GeneralLedgerReportPartner(models.TransientModel):
    ...
    _name = 'report_general_ledger_partner_contabilidad_cfdi'
    _inherit = 'account_financial_report_contabilidad_cfdi_abstract'
...
ORDER BY
    CASE
        WHEN "report_general_ledger_partner_contabilidad_cfdi"."partner_id" IS NOT NULL
        THEN 0
        ELSE 1
    END,
    "report_general_ledger_partner_contabilidad_cfdi"."name"
        """


class GeneralLedgerReportMoveLine(models.TransientModel):

    _name = 'report_general_ledger_move_line_contabilidad_cfdi'
    _inherit = 'account_financial_report_contabilidad_cfdi_abstract'
...
class GeneralLedgerReportCompute(models.TransientModel):
    ...
    _inherit = 'report_general_ledger_contabilidad_cfdi'

    @api.multi
    def print_report(self, report_type):
        self.ensure_one()
        if report_type == 'xlsx':
            report_name = 'a_f_r.report_general_ledger_xlsx_contabilidad_cfdi'
        else:
            report_name = 'contabilidad_cfdi.' \
                          'report_general_ledger_qweb_contabilidad_cfdi'
    ...
    def _get_html(self):
        ...
        if report:
            ...
            result['html'] = self.env.ref(
                'contabilidad_cfdi.report_general_ledger_contabilidad_cfdi').render(
                    rcontext)
        return result
    ...
    def _get_account_sub_subquery_sum_amounts(
        ...
        INNER JOIN
            ...
            """
            if self._context.get('is_contabilidad_electronica'):
                sub_subquery_sum_amounts += """ AND m.contabilidad_electronica"""
        elif self._context.get('is_contabilidad_electronica'):
            sub_subquery_sum_amounts += """
        INNER JOIN
            account_move m ON ml.move_id = m.id AND m.contabilidad_electronica
            """
...
INSERT INTO
    report_general_ledger_account_contabilidad_cfdi
    (
    report_id,
    create_uid,
    create_date,
...
AND
    ...
    def _inject_partner_values(self, only_empty_partner=False):
        """ Inject report values for report_general_ledger_partner_contabilidad_cfdi.
...
WITH
    accounts_partners AS
        (
            ...
            FROM
                report_general_ledger_account_contabilidad_cfdi ra
            INNER JOIN
    ...
    final_sum_amounts AS ( """ + final_subquery + """ )
INSERT INTO
    report_general_ledger_partner_contabilidad_cfdi
    (
    report_account_id,
    create_uid,
    create_date,
...
AND
    ...
    def _inject_line_not_centralized_values(
        ...
        """ Inject report values for report_general_ledger_move_line_contabilidad_cfdi.
        ...
        WITH
            ...
            if is_account_line:
                ...
                query_inject_move_line += """
                        report_general_ledger_partner_contabilidad_cfdi rp
                    INNER JOIN
                        report_general_ledger_account_contabilidad_cfdi ra
        ...
        query_inject_move_line += """
INSERT INTO
    report_general_ledger_move_line_contabilidad_cfdi
    (
        """
        if is_account_line:
            query_inject_move_line += """
...
FROM
    ...
    report_general_ledger_partner_contabilidad_cfdi rp
INNER JOIN
    report_general_ledger_account_contabilidad_cfdi ra ON rp.report_account_id = ra.id
            """
        query_inject_move_line += """
INNER JOIN
    ...
    (
        (
            ...
            FROM
                account_move_line ml
            INNER JOIN
                report_general_ledger_move_line_contabilidad_cfdi rml
...
WHERE
    report_general_ledger_move_line_contabilidad_cfdi.id = tags_values.report_id
            """
        params = {
            'report_id': self.id,
        }
    ...
    def _inject_unaffected_earnings_account_values(self):
        ...
        for report_general_ledger_account_contabilidad_cfdi."""
...
"""
        ...
        if self.only_posted_moves:
            query_select_previous_fy_unaffected_earnings += """
                AND am.state = 'posted'
            """
            query_select_previous_fy_unaffected_earnings += """ AND am.contabilidad_electronica"""
        elif self._context.get('is_contabilidad_electronica'):
            query_select_previous_fy_unaffected_earnings += """ AND am.contabilidad_electronica"""
