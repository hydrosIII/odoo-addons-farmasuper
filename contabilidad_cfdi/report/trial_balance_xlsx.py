...
class TrialBalanceXslx(models.AbstractModel):
    ...
    def _get_report_name(self, report):
        report_name = _('Balanza de comprobación')
        return self._get_report_complete_name(report, report_name)

    def _get_report_columns(self, report):
        if not report.show_partner_details:
            res = {
                0: {'header': _('Código'), 'field': 'code', 'width': 10},
                1: {'header': _('Nombre de la cuenta'), 'field': 'name', 'width': 60},
                2: {'header': _('Balance incial'),
                ...
                4: {'header': _('Crédito'),
                    ...
                    'type': 'amount',
                    'width': 14},
                5: {'header': _('Balance en el periodo'),
                    'field': 'period_balance',
                    'type': 'amount',
                    'width': 14},
                6: {'header': _('Balance final'),
                    'field': 'final_balance',
                    'type': 'amount',
                    'width': 14},
            }
    ...
    def _get_report_filters(self, report):
        return [
            [_('Rango de fechas'),
             _('De: %s Hasta: %s') % (report.date_from, report.date_to)],
            [_('Target moves filter'),
             _('All posted entries') if report.only_posted_moves else _(
                 'Todos los asientos')],
            [_('Cuentas con 0'),
             _('No mostrar') if report.hide_account_at_0 else _('Mostrar')],
            [_('Mostrar moneda extranjera'),
             _('Si') if report.foreign_currency else _('No')],
            [_('Limitar los niveles'),
             _('Nivel %s' % report.show_hierarchy_level) if
             report.limit_hierarchy_level else _('Sin límite')],
        ]
